...
<body>
  ...
  if (isset($_POST['update'])) {
    ...
    try {
      $stmt = $conn->prepare("UPDATE complaint SET status=?, remarks=?, remarks_date=NOW() WHERE id=?");
    ...
    if ($query_run) {
      echo '<script>alert("Status Updated"); window.location="complaintList.php";</script>';
    } else {
      ...
      <div class="row justify-content-center">
        <div class="col-md-8">
          <div class="card-custom">
            <div class="card-header orange"><i class="fas fa-exclamation-circle"></i> Complaint #<?php echo str_pad($id, 4, '0', STR_PAD_LEFT); ?></div>
            <div class="card-body">
              <form method="POST" class="form-custom">
                <div class="form-row">
                  <div class="form-group col-md-6">
                    <label>Account Name</label>
                    <input type="text" class="form-control" value="<?php echo htmlspecialchars($rows['name']); ?>" readonly>
                  </div>
                  <div class="form-group col-md-6">
                    <label>Account Number</label>
                    <input type="text" class="form-control" value="<?php echo htmlspecialchars($rows['accountnumber']); ?>" readonly>
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" value="<?php echo htmlspecialchars($rows['complaint']); ?>" readonly>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  <label>Remarks</label>
                  <textarea class="form-control" name="remarks" rows="4" placeholder="Add remarks or notes..."><?php echo htmlspecialchars($rows['remarks'] ?? ''); ?></textarea>
                </div>
                <div class="text-center mt-4">
                  <button type="submit" name="update" class="btn-pw btn-pw-orange btn-pw-lg"><i class="fas fa-save"></i> Update Status</button>
                  <a href="complaintList.php" class="btn-pw btn-pw-outline btn-pw-lg"><i class="fas fa-times"></i> Cancel</a>
                </div>
              </form>
            </div>
          </div>
